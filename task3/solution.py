...
def load_data(**kwargs):
    ...
    # NEW - ADDED: remove center => train with same parts removed
...
def training(train_data_input, train_data_label, **kwargs):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3) #torch.optim.SGD(model.parameters())
...
class Model(nn.Module):
    ...
    def __init__(self, base_ch = 32):
        ...
        # Encoder
        self.enc1 = DoubleConv(1, base_ch)           # [B,  32, 28, 28]
        self.pool1 = nn.MaxPool2d(2)                 # [B,  32, 14, 14]
        self.enc2 = DoubleConv(base_ch, base_ch * 2) # [B,  64, 14, 14]
        self.pool2 = nn.MaxPool2d(2)                 # [B,  64,  7,  7]

        # Bottleneck
        self.bottleneck = DoubleConv(base_ch * 2, base_ch * 4)  # [B, 128, 7, 7]

        # Decoder
        self.up2 = nn.ConvTranspose2d(base_ch * 4, base_ch * 2, kernel_size=2, stride=2)  # [B, 64, 14, 14]
        self.dec2 = DoubleConv(base_ch * 4, base_ch * 2)        # concat → 128 ch in
        self.up1 = nn.ConvTranspose2d(base_ch * 2, base_ch, kernel_size=2, stride=2)  # [B, 32, 28, 28]
        self.dec1 = DoubleConv(base_ch * 2, base_ch)            # concat → 64 ch in
...
def testing(model, test_data_input):
    ...
    with torch.no_grad():
        ...
        batch_size = 128     # NEW - CHANGE: changed to 128 from 16 for efficiency, can be changed back
        for i in tqdm(
    ...
    # Save the output
    test_data_output = test_data_output.numpy() * 255       # NEW - times 255 to cancel out the normalization
    ...
    if True:
        ...
        for i in tqdm(range(20), desc="Plotting test images"):
            ...
            plt.subplot(1, 2, 2)
            plt.imshow(test_data_output[i].squeeze(), cmap="gray", vmin=0, vmax=255)    #plt.imshow(test_data_output[i].squeeze(), cmap="gray")
